...
class PurchaseRequestLineMakePurchaseOrder(models.TransientModel):
    ...
    def _prepare_item(self, line):
        return {
            ...
            "estimated_qty": line.product_qty,
    ...
    def _prepare_purchase_order_line(self, po, item):
        ...
        estimated_qty = item.line_id.product_qty
    ...
    def make_purchase_order(self):
        ...
        for item in self.item_ids:
            ...
            # po line
            _logger.info(f"linesssssssssss : {line.request_id}")
            _logger.info(f"linesssssssssss : {line.request_id.project_id}")
            _logger.info(f"linesssssssssss : {line.request_id.task_id}")
            _logger.info(f"linesssssssssss : {line.request_id.sale_order_id}")
            purchase.write({
                'state' : 'draft',
                'delivery_method' : self.delivery_method,
                'fleet_id' : self.fleet_id,
                'date_pickup' : self.date_pickup,
                'project_id' : line.request_id.project_id.id,
                'task_id' : line.request_id.task_id.id,
                'sale_order_id' : line.request_id.sale_order_id.id,
            })
            ...
            date_planned = self._get_date_with_user_tz(date_required)
            po_line.write({"product_qty": item.product_qty, "date_planned": date_planned, "estimated_qty" : line.product_qty})
            _logger.info(f"new linesss : {line}")
            _logger.info(f"new qty : {line.product_qty}")
            _logger.info(f"new qty : {item.estimated_cost}")
            _logger.info(f"est qty : {po_line}")
